...
video=video||curVideo;if(!video instanceof tvp.VideoInfo){throw new Error("浼犲叆鐨勫璞′笉鏄痶vp.VideoInfo鐨勫疄渚�");}
...
return;el.innerHTML=getVideoTagHtml();videoObj=tvp.$.get(playerid);$videoTag=tvp.$(videoObj);$me.onwrite(curVideo.getChannelId());$me.oninited();this.play(curVideo,false);};this.play=function(video,hookOnChage){if(!videoObj){throw new Error("鏈壘鍒拌棰戞挱鏀惧櫒瀵硅薄锛岃纭<video>鏍囩鏄惁瀛樺湪");}
if(!video instanceof tvp.VideoInfo){throw new Error("浼犲叆鐨勫璞′笉鏄痶vp.VideoInfo鐨勫疄渚�");}
...
tvp.MP4Link.prototype=new tvp.BasePlayer();tvp=tvp||{};var QQLive=QQLive||{ver:"$Rev: 41363 $",curSSO:"",lastmodify:"$Date: 2014-01-24 14:40:13 +0800 (浜�, 2014-01-24) $"};var QQLiveSetup=QQLive;QQLive.log=tvp.log;QQLive.debug=function(msg){var funName="";if(msg.indexOf("[")<0&&!!arguments&&!!arguments.callee&&!!arguments.callee.caller){var caller=arguments.callee.caller;funName=QQLive.debug.getFunName(caller)}
...
function showInitOcxFail(container,errcode){var msg="";if(!tvp.$.isUndefined(errcode)){msg="<br/>閿欒鐮�:OCX_"+QQLive.DEFINE.MSG.OCX_DISABLED+"_"+errcode;}
...
tvp.$.get(containerId).style.backgroundColor="#000000";if(!tvp.$.userAgent.windows){tvp.OcxPlayer.showNoticeTips(container,{title:"鎮ㄧ殑鎿嶄綔绯荤粺鏆備笉鏀寔鑵捐瑙嗛鎾斁鍣�",width:this.width,height:this.height});return;}
if(!tvp.$.userAgent.isCanOcx()){tvp.OcxPlayer.showNoticeTips(container,{title:"鎮ㄧ殑娴忚鍣ㄦ殏涓嶆敮鎸佽吘璁棰戞挱鏀惧櫒",width:this.width,height:this.height});return;}
